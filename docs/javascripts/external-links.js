(function () {
  ...
  function ensureHeaderDiscord() {
    ...
    link.title = document.documentElement.lang === "es" ? "Ayuda en Discord (abre otra pestaña)" : "Support on Discord (opens in a new tab)"
  ...
  function bindLinks(root) {
    root.querySelectorAll("a[href]").forEach(function (link) {
      ...
      if (!isExternal(href) || link.hasAttribute("hreflang") || new URL(href, window.location.href).origin === window.location.origin) {
        return
      }
      link.target = "_blank"
      link.rel = "noopener noreferrer"
